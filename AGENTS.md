...
- `shiptalkers-app/src/website/mod.rs` - axum router, server-rendered `/stats`, `/stats/:id` (user or channel, dispatched by `U`/`C` prefix), `/leaderboard` and `/search` via askama; `/pfp/:id` looks up the stored Slack pfp URL and redirects to it. The per-user page shows a link prompt (pointing at `/link`) whenever a user has no coding total or their hackatime profile is private; linking is never required for anything else
- `shiptalkers-app/src/website/auth.rs` - hackatime OAuth login/callback/disconnect plus `sync_coding_activity` (fetches the user's coding spans from the spans endpoint over an incremental window, with the OAuth token when present, stores them with exact timestamps in `hackatime_spans`, and rewrites `hackatime_connections.total_minutes` from the span sum; returns `SyncFailure` with `PrivateProfile`/`NoAccount`/`Message`) and the 30m `resync_all` loop over every non-bot user (token users via OAuth, everyone else via the public spans endpoint, skipping `no_account` users for 30 days and `private` users with no token)
...
- ClickHouse is the only analytics datastore. The stats page reads `slack_messages`, `slack_channels`, `hackatime_connections`, `daily_stats`, and `user_scores`; per-user pages read `user_scores` (plus `slack_messages_by_user` for the top-channels list). SQLite (`shiptalkers-app/src/db/sqlite.rs`) holds auth/linked-user state.
- Insert data before marking any checkpoint complete. Main channel messages are inserted before thread replies.
- Progress tracking uses `max(message_ts)` per channel and `max(thread reply ts)` per thread. `slack_messages.message_ts` is `UInt64` microseconds (Slack sends "seconds.microseconds" strings, converted via `slack_ts_to_micros`/`micros_to_slack_ts` in `shiptalkers-app/src/db/clickhouse_db.rs`); `thread_ts` stays `String` because Slack pagination uses it verbatim.
...
- Coding time is stored per user per span in `hackatime_spans`, with one total per user in `hackatime_connections.total_minutes` derived from the span sum (Top Coders / Top Combined / per-user pages read the total; the stats bot card sums each span's exact overlap with the requested range). Coding syncs are serialized per user (`CODING_SYNC_LOCKS` in `auth.rs`). The fetch runs before any DB write, so a failed sync leaves the old data intact; a 401/403 from the spans endpoint only deletes the connection if the `me` endpoint confirms the token is dead, so a hackatime outage (5xx or transport errors) never strips links or forces re-linking from `/link`. A 403 on the unauthenticated path means the profile is private (`private` state) and a 404 means there is no account for that Slack UID (`no_account` state); both are written back to `hackatime_connections` with an empty `access_token` so the resync loop skips them until a token appears or the `no_account` 30-day retry window passes.
