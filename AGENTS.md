...
- `api.rs` - the implemented JSON handlers: `list_api_keys` / `create_api_key` / `revoke_api_key` (session-authenticated key management) and `get_me` (bearer-key-authenticated, loads the key owner's stats via `load_user_stats`). Errors are `{"error": "..."}`. Key management is also reachable from the `/link` account tab (`link_create_api_key`, `link_revoke_api_key` in `auth.rs`), which shows a new key exactly once. State-changing calls must present a CSRF token derived from the signed session cookie (HMAC over the session payload, `auth::csrf_token`/`csrf_ok`): HTML forms carry it in a hidden `csrf` field, JSON/API clients read the `st_csrf` cookie and send it in `X-CSRF-Token`.
