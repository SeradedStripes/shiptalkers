...
Coding time for every non-bot, non-deleted user is pulled from hackatime on a 30-minute cycle as one total per user (there is no per-day coding data anymore). Most users sync through the public per-user stats endpoint (keyed by Slack UID, no OAuth); a user whose hackatime profile is not public gets a `private` flag and only syncs once they link via OAuth. Missing profiles are recorded as `no_account` and retried every 30 days. A user with a saved OAuth token always syncs through it.
...
- `shiptalkers-app/src/website/auth.rs` - hackatime OAuth login/callback/disconnect plus `sync_coding_activity` (fetches the user's total coding minutes from the stats endpoint, with the OAuth token when present, and writes it to `hackatime_connections`; returns `SyncFailure` with `PrivateProfile`/`NoAccount`/`Message`) and the 30m `resync_all` loop over every non-bot user (token users via OAuth, everyone else via the public stats endpoint, skipping `no_account` users for 30 days and `private` users with no token)
...
- Coding time is one total per user stored in `hackatime_connections.total_minutes` (there is no per-day coding data anymore, so the Top Coders / Top Combined / per-user pages / stats bot all read that total). Coding syncs are serialized per user (`CODING_SYNC_LOCKS` in `auth.rs`). The fetch runs before any DB write, so a failed sync leaves the old total intact; a 401/403 from the stats endpoint only deletes the connection if the `me` endpoint confirms the token is dead, so a hackatime outage (5xx or transport errors) never strips links or forces re-linking from `/link`. A 403 on the unauthenticated path means the profile is private (`private` state) and a 404 means there is no account for that Slack UID (`no_account` state); both are written back to `hackatime_connections` with an empty `access_token` so the resync loop skips them until a token appears or the `no_account` 30-day retry window passes.
