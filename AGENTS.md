...
- They are same-row views: the per-user Slack Time queries read `slack_messages` directly, served by the `slack_messages_user_ts_idx (user_id, message_ts)` index, so no denormalized per-user copy of every message is kept. `slack_messages`, `word_counts`, `users.pfp`, and `slack_reactions.emoji` use zstd TOAST compression (`SET COMPRESSION zstd`, Postgres 18+); the scraper's `compact_toast_once` startup task rewrites the heavy tables once with `VACUUM FULL` to recompress existing rows (tracked in `backfill_meta`, skipped while they are tiny).
