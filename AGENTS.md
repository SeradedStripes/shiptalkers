...
- The `users` table keeps four separate name fields: `merged_name` (the display name everywhere on the site and API, merged as Slack display name, else real name, else handle), `display_name` (strict Slack display name/nickname), `real_name` (full name), and `username` (the `@handle`), plus `email`. `init_tables` migrates old `display_name` rows by renaming the column to `merged_name`; the 2h `users.list` sync re-fetches any non-bot user still missing `real_name`/`username` until they are populated. `email` only appears if the Slack app scope provides it.
