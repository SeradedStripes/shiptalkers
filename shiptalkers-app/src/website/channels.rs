...
pub(super) async fn get_channels_board(
    state: State<AppState>,
    headers: super::HeaderMap,
    Query(params): Query<HashMap<String, String>>,
) -> Result<Html<String>, StatusCode> {
    render_channels_board(state, headers, Query(params)).await
}

async fn render_channels_board(
    ...
    let total: i64 = super::sqlx::query_scalar("SELECT count(*) FROM slack_channels")
        .fetch_one(ch)
        .await
        .unwrap_or(0);
