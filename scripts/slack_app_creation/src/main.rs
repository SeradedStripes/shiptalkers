...
fn app_env_entries(existing: Option<&str>, bot_token: &str) -> Vec<EnvEntry> {
    vec![
        EnvEntry {
...
async fn run() -> Result<(), String> {
    ...
    write_env_output(&app_env_entries(fs::read_to_string(output_env_path()).ok().as_deref(), &bot_token))?;
...
mod tests {
    ...
    fn writes_fresh_env_output_with_header() {
        let entries = app_env_entries(None, "xoxb-a");
    ...
    fn rerun_uses_next_numbered_key() {
        ...
        let entries = app_env_entries(Some(existing), "xoxb-2");
    ...
    fn rerun_keeps_user_filled_empty_values() {
        let entries = app_env_entries(None, "xoxb-new");
