    ...
    for entry in entries {
        ...
        for line in lines.iter_mut() {
            ...
            if key.trim() == entry.key {
                if entry.append && !entry.value.is_empty() {
                    if !value.split(',').any(|v| v.trim() == entry.value) {
                        *line = format!("{}={},{}", entry.key, value, entry.value);
                    }
...
fn app_env_entries(bot_token: &str, user_token: &str) -> Vec<EnvEntry> {
    vec![
        EnvEntry {
            comment: "# Bot token(s), comma-separated for multiple apps (SLACK_BOT_TOKENS).",
            key: "SLACK_BOT_TOKENS",
            value: bot_token.to_string(),
            append: true,
        },
        EnvEntry {
            comment: "# User token(s), comma-separated for multiple apps (SLACK_USER_TOKENS).",
            key: "SLACK_USER_TOKENS",
            value: user_token.to_string(),
            append: true,
        },
        EnvEntry {
            comment: "# App token (xapp-...), created under the app's App-Level Tokens page.",
            key: "SLACK_APP_TOKENS",
            value: String::new(),
            append: false,
        },
    ]
...
async fn run() -> Result<(), String> {
    ...
    write_env_output(&app_env_entries(&bot_token, &user_token))?;
...
mod tests {
    ...
    fn writes_fresh_env_output_with_header() {
        let entries = app_env_entries("xoxb-a", "xoxp-b");
        let out = merge_env_output(None, &entries);
        assert!(out.starts_with("# Auto-generated by slack_app_creation"));
        assert!(out.contains("SLACK_BOT_TOKENS=xoxb-a\n"));
        assert!(out.contains("SLACK_USER_TOKENS=xoxp-b\n"));
        assert!(out.contains("SLACK_APP_TOKENS=\n"));
    }

    #[test]
    fn rerun_appends_new_tokens_and_new_keys() {
        let entries = app_env_entries("xoxb-2", "xoxp-2");
        let existing = "SLACK_MAIN_CHANNEL=C123\nSLACK_BOT_TOKENS=xoxb-1\nSLACK_USER_TOKENS=xoxp-1\n";
        let out = merge_env_output(Some(existing), &entries);
        assert!(out.contains("SLACK_BOT_TOKENS=xoxb-1,xoxb-2\n"));
        assert!(out.contains("SLACK_USER_TOKENS=xoxp-1,xoxp-2\n"));
        assert!(out.contains("SLACK_MAIN_CHANNEL=C123\n"));
        assert!(out.contains("SLACK_APP_TOKENS=\n"));
    }

    #[test]
    fn rerun_skips_duplicate_tokens() {
        let entries = app_env_entries("xoxb-1", "xoxp-1");
        let existing = "SLACK_BOT_TOKENS=xoxb-1\n";
        let out = merge_env_output(Some(existing), &entries);
        assert!(out.contains("SLACK_BOT_TOKENS=xoxb-1\n"));
        assert!(!out.contains("xoxb-1,xoxb-1"));
    }

    #[test]
    fn rerun_keeps_user_filled_empty_values() {
        let entries = app_env_entries("xoxb-new", "xoxp-new");
